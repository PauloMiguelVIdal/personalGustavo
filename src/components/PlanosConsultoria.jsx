...
const PlanosConsultoria = () => {
  ...
  return (
    <div id="planos" className="bg-gradient-to-b from-black via-gray-900 to-black py-20 px-4">
      <div className="max-w-7xl mx-auto">
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {planos.map((plano, index) => (
            <div 
              key={index}
              className={`relative group ${plano.popular ? 'md:scale-110 z-10' : ''}`}
            >
              {plano.popular && (
              ...
              <div className={`relative bg-gradient-to-br ${plano.gradient} p-8 rounded-3xl overflow-hidden transition-all duration-500 hover:scale-105 hover:shadow-2xl hover:shadow-orange-500/30`}>
                <div className="absolute inset-0 bg-black/20"></div>
                
                <div className="relative z-10 text-center">
                  <div className="inline-block bg-black/40 backdrop-blur-sm px-6 py-2 rounded-full mb-6">
                    <h2 className="text-2xl font-black text-white">{plano.titulo}</h2>
                  </div>
                  
                  <h3 className="text-xl font-bold text-white mb-4">{plano.descTitle}</h3>
                  
                  <p className="text-white/90 text-sm mb-8 leading-relaxed">{plano.desc}</p>
